...
struct sdp_storage * sdp_get_storage(int storage_id)
{
	...
	list_for_each_entry(tmp_storage, &current_sdp_user->storage_list, list) {
		if (tmp_storage->storage_id == storage_id) {
			SDP_LOGI("storage %d is found\n", storage_id);
			return tmp_storage;
		}
	}
	SDP_LOGE("storage %d is not found\n", storage_id);
	return NULL;
}
...
int sdp_file_set_protected(struct dentry *dentry) {
	...
	if(S_ISDIR(inode->i_mode)) {
		...
		*/
		...
		if (rc) {
			SDP_LOGE("Error computing the root IV\n");
			goto out;
		}
...
static long sdp_do_ioctl_ctl(unsigned int minor, unsigned int cmd, unsigned long arg) {
	...
	case SDP_ON_DEVICE_UNLOCKED: {
		...
		if (ret < 0) {
			SDP_LOGE("Failed to unlock %d storage, ret = %ld", params->storage_id, ret);
			goto err;
	...
	case SDP_ON_DEVICE_LOCKED: {
		...
		ret = sdp_storage_lock(current_sdp_user, params->storage_id);
		if (ret < 0) {
			SDP_LOGE("Failed to unlock %d storage, ret = %ld", params->storage_id, ret);
			goto err;
		} else if (ret == SDP_NOT_EXIST) {
	...
	case SDP_ON_DEL_USER: {
		...
		if (ret < 0) {
			...
			SDP_LOGE("User %d dose not exist", params->user_id);
		} else {
