...
static int ftm4_jitter_test(struct device *dev)
{
	char file_buf[FILE_BUF_SIZE] = {0};
	...
	s16 jitter_limit = 0;
	s16 jitter_min_limit = 0;
	s16 jitter_max_limit = 0;
	s16 frame_cnt = 0;
	s16 jitter_min = 10000;
	s16 jitter_max = -10000;
	int jitter_min_index[2] = {0};
	int jitter_max_index[2] = {0};
	int i = 0;
	int j = 0;
	int ret = 0;
	...
	TOUCH_I("=== Jitter Test Start ===\n");

	snprintf(file_buf, sizeof(file_buf), "[JITTER_TEST]\n\n");
	ftm4_write_file(dev, file_buf, TIME_INFO_SKIP);

	ftm4_get_limit(dev, "JITTER_TOTAL_FRAME_CNT", &total_frame_cnt, 1, 1);
	ftm4_get_limit(dev, "JITTER_LIMIT", &jitter_limit, 1, 1);
	jitter_min_limit = -jitter_limit;
	jitter_max_limit = jitter_limit;
	...
	for (frame_cnt = 0; frame_cnt < total_frame_cnt; frame_cnt++) {
		...
		for (i = 0; i < FORCE_CH_SIZE; i++) {
			for (j = 0; j < SENSE_CH_SIZE; j++) {
				if ((rawdata[i][j] < jitter_min_limit) ||
					(rawdata[i][j] > jitter_max_limit)) {
					result = 1;
					memset(file_buf, 0, sizeof(file_buf));
				...
				if (rawdata[i][j] < jitter_min) {
					jitter_min = rawdata[i][j];
					jitter_min_index[0] = i;
					jitter_min_index[1] = j;
				}

				if (rawdata[i][j] > jitter_max) {
					jitter_max = rawdata[i][j];
					jitter_max_index[0] = i;
					jitter_max_index[1] = j;
				}
			}
		}
	}
	...
	ret += snprintf(file_buf + ret, FILE_BUF_SIZE - ret,
			"jitter_min_limit = %d , jitter_max_limit = %d\n",
			jitter_min_limit, jitter_max_limit);
	TOUCH_I("%s: jitter_min_limit = %d , jitter_max_limit = %d\n",
			__func__, jitter_min_limit, jitter_max_limit);
	ret += snprintf(file_buf + ret, FILE_BUF_SIZE - ret,
			"jitter_min = %d [%d][%d] , jitter_max = %d [%d][%d]\n",
			jitter_min, jitter_min_index[0], jitter_min_index[1],
			jitter_max, jitter_max_index[0], jitter_max_index[1]);
	TOUCH_I("%s: jitter_min = %d [%d][%d] , jitter_max = %d [%d][%d]\n",
			__func__,
			jitter_min, jitter_min_index[0], jitter_min_index[1],
			jitter_max, jitter_max_index[0], jitter_max_index[1]);
	...
	memset(file_buf, 0, sizeof(file_buf));
	snprintf(file_buf, sizeof(file_buf), "\nJITTER_TEST Result : %s\n\n",
			(result == 0) ? "Pass" : "Fail");
	ftm4_write_file(dev, file_buf, TIME_INFO_SKIP);

	TOUCH_I("JITTER_TEST Result: %s\n", (result == 0) ? "Pass" : "Fail");

	TOUCH_I("=== Jitter Test End ===\n");
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	jitter_ret = ftm4_jitter_test(dev);
