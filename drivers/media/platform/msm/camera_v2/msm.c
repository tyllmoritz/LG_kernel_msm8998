...
void msm_delete_stream(unsigned int session_id, unsigned int stream_id)
{
	...
	unsigned long flags;
	unsigned long flags_rw;/*fix the watchdog bite, 2017-07-11, Camera-Stability@lge.com*/
	int try_count = 0;
	...
	while (1) {
		...
		write_lock_irqsave(&session->stream_rwlock, flags_rw);/*fix the watchdog bite, 2017-07-11, Camera-Stability@lge.com*/
		try_count++;
		...
		if (!stream) {
			...
			write_unlock_irqrestore(&session->stream_rwlock, flags_rw);/*fix the watchdog bite, 2017-07-11, Camera-Stability@lge.com*/
			return;
		}
