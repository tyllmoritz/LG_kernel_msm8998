...
#if defined(CONFIG_LGE_DISPLAY_COLOR_MANAGER)
	if (!ctrl_pdata->lge_ctrl_pdata->is_backup) {
		ret = lge_color_manager_reg_backup(ctrl_pdata);

		if (ret < 0)
			pr_err("[Display] %s: fail to backup CM reg.\n", __func__);
		else
			ctrl_pdata->lge_ctrl_pdata->is_backup = true;
	}
#endif

#if defined(CONFIG_LGE_DISPLAY_AMBIENT_SUPPORTED)
	if (!ctrl_pdata->ambient_reg_backup) {
		lge_mdss_ambient_backup_internal_reg(ctrl_pdata);
...
static bool mdss_dsi_drs_retry_switch_panel_cfg(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	while (retry_cnt > 0) {
		...
		mdelay(16);
		lge_mdss_dsi_panel_cmd_read(0x3F, 0x01, &data);
