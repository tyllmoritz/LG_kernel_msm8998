...
int32_t lgpki_send_req_command(uintptr_t ka, uintptr_t ks, uintptr_t scta)
{
	...
	int32_t req_len = 0;
	int32_t rsp_len = 0;
	struct lgpki_send_cmd *req;      // request data sent to QSEE
	struct lgpki_send_cmd_rsp *rsp;  // response data sent from QSEE
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    switch(cmd) {
        case CLIENT_CMD11_SET_KERNEL_INFO:
            {
                if (pkm_flag == 0) {
                    /* Get Physical Address of _text */
                    kernel_x_start_rounded = round_down(__pa(_stext),PAGE_SIZE);
                    kernel_x_end = round_up(__pa(__end_rodata), PAGE_SIZE);
                    /* Get physical address and size of kernel
                     * 1. convert pointer into string by using sprintf
                     * 2. convert string into hex by using sscanf
                     * 3. get PA and size */
                    ...
                    sscanf(phys_e, "%lx", &pe);
                    ks = pe - ps;
                    /* Get sys_call_table Virtual Address */
                    sct_p = (uintptr_t)(sys_call_table);
                    /* Request LGPKI TZ App to save kernel address information */
                    ret = lgpki_start();
                    if (ret) {
...
static int __init lge_pkm_probe(struct platform_device *pdev)
{
	...
	/* create /sys/class/lge_pkm/pkm_ctrl/pkm_command */
